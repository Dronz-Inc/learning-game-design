...
> **Status — Phase 5a built.** Playable prototype at `assignment-1/roll-call.html` (self-contained,
> open in any browser). Press **Start** → crypto-RNG dice (unbiased d6 via rejection sampling) pick the
> round's three criteria → type a country and submit → correct extends the streak and auto-rolls the
> next round; a valid-but-wrong answer ends the game, reveals the round's valid answers and final score,
> and offers **Play again**. Unrecognised spellings prompt a retry (no penalty); accepts aliases
> (USA, UK, DRC, Côte d'Ivoire…) and is accent/case-insensitive. Generated by
> `criteria-calibration/build_game.py` from the locked criteria + `country_data.csv`; embeds the answer
> data only for digital checking — the printable PDF (5b) stays answer-free.
